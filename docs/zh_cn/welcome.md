...
3. 当存在**功能不全**, **无法进入应用**或者**可复现的bug**时, 可尝试下载最新版本对应架构的`MAH-XXX-XXX-incremental-vXXX.zip`压缩包并把压缩包内的**内容物**覆盖到**应用根目录**, 如果仍然存在对应的问题, 请通过Github的[`Issue界面`](https://github.com/Quartewe/MAH/issues), 即`https://github.com/Quartewe/MAH/issues`, 按照流程提供对应的内容以获得最快的解决。
...
- 选择队伍：填入对应的队伍ID, 即每个队伍栏左上角的那个`team X`, 只用填**数字**
...
- 关卡选择：支持14章花本、金币/经验/技能副本及突破界限试炼，活动关卡支持且`推荐`手动填写部分关键词
...
自动检查任务进度, 当未完成本周的`显示助战页面`任务时会自动完成
...
从`战友积分`选项卡点击进入, 根据右侧的`我应该怎么做`下方的内容完成对应的操作后, 输入抽几次10连, 确认后将开始操作, 可随时停止

## 相关内容查找方式

从`资源索引`选项卡点击进入, 既可搜寻到角色以及ar卡索引
