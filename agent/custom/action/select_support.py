...
class SelectSupport(CustomAction):
    ...
    def run(
        ...
        if isinstance(param, str):
            ...
            try:
                ...
                if raw_data:
                    ...
                    logger.info(f"助战数据: {support_data}")
                else:
                    support_data = None
        ...
        if not self._scan_and_select_support(context, support_data if not default_mode else None, keywords, select_mode, idroi):
            ...
            logger.info(f"助战名称: {support_name}, 编号: {id_format}")
            support_details = self.CHAR_DATA.get(support_name, {}).get(id_format, {})
            logger.info(f"助战详情: {support_details}")
            element = support_details.get("element", "")
            logger.info(f"属性: {element}")
            ...
            logger.info(f"未找到精确匹配，尝试按属性筛选: {element_filter}")
            if element_filter:
                context.run_task(
                    "UtilsTemplateMatch",
                    pipeline_override={
        ...
        # 滑动到对应位置
        temp = 0
        logger.info(f"需要滑动 {swipe_time} 次以到达目标位置")
                ...
                for name, id_dict in add_res.items():
                    ...
                    for char_id, res_dict in id_dict.items():
                        ...
                        for res_key, res_data in res_dict.items():
                            ...
                            logger.info(f"正在处理结果 {res_key}，box={res_box}")
                            for i, fp in enumerate(fpbox):
                                logger.info(f"检查指纹框 {fp} 与结果框 {res_box} 的关系")
                                if act_mgr.in_roi(res_box, fp):
                                    res_data["pos"] = i + page
                                    logger.info(f"结果框 {res_box} 位于指纹框 {fp} 内")
                                    break
                                else:
