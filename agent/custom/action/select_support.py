...
class SelectSupport(CustomAction):
    ...
    def run(
        ...
        if isinstance(param, str):
            ...
            try:
                ...
                if raw_data:
                    ...
                    logger.debug(f"助战数据: {support_data}")
                else:
                    support_data = None
        ...
        if not support_data:
            logger.warning(f"目标文件中未找到助战数据，使用默认数据")
            default_mode = True
            support_data = {
                "name": "kyouma",
                "id": 2
        ...
        if not self._scan_and_select_support(context, support_data if not default_mode else None, keywords, select_mode, idroi):
            ...
            temp = 0
            while temp < (page // 3):
                logger.debug(f"正在滑动到顶部，进度 {temp}/{(page // 3)}")
                context.run_action(
                    "UtilsSwipe",
                    pipeline_override={
                        "UtilsSwipe": {
            ...
            logger.debug(f"助战名称: {support_name}, 编号: {id_format}")
            support_details = self.CHAR_DATA.get(support_name, {}).get(id_format, {})
            logger.debug(f"助战详情: {support_details}")
            element = support_details.get("element", "")
            logger.debug(f"属性: {element}")
            ...
            logger.debug(f"未找到精确匹配，尝试按属性筛选: {element_filter}")
            if element_filter:
                context.run_task(
                    "UtilsTemplateMatch",
                    pipeline_override={
        ...
        # 滑动到对应位置
        temp = 0
        logger.debug(f"需要滑动 {swipe_time} 次以到达目标位置")
        while temp < swipe_time:
            logger.debug(f"正在滑动到目标位置，进度 {temp}/{swipe_time}")
            context.run_action(
                "UtilsSwipe",
                pipeline_override={
                    "UtilsSwipe": {
    ...
    def _scan_and_select_support(self, context, support_data, keywords, select_mode, idroi):
        ...
        while True:
            ...
            if not recres:
                ...
                if rbox is None:
                    logger.debug(f"未找到角色: {support_data['name']} {support_data['id']}")
                    add_res = {}
                else:
                    ...
                    if rbox == [0, 0, 1, 1]:
                        logger.debug(f"找到多个角色")
                    else:
                        logger.debug(f"找到一个角色: {support_data['name']} {support_data['id']}")
                    
            # 为每个 res_* 添加 pos 信息
            if add_res:
                logger.debug(f"已更新识别结果: {add_res}")
                for name, id_dict in add_res.items():
                    ...
                    for char_id, res_dict in id_dict.items():
                        ...
                        for res_key, res_data in res_dict.items():
                            ...
                            logger.debug(f"正在处理结果 {res_key}，box={res_box}")
                            for i, fp in enumerate(fpbox):
                                logger.debug(f"检查指纹框 {fp} 与结果框 {res_box} 的关系")
                                if act_mgr.in_roi(res_box, fp):
                                    res_data["pos"] = i + page
                                    logger.debug(f"结果框 {res_box} 位于指纹框 {fp} 内")
                                    break
                                else:
                                    logger.debug(f"结果框 {res_box} 不在指纹框 {fp} 内")
                self.all_res = match_mgr.merge_res_dicts(self.all_res, add_res)
            
            logger.debug(f"第 {page // 3} 页，指纹: {current_fingerprint}，正在滑动...")
            context.run_action(
                "UtilsSwipe",
                pipeline_override={
                    "UtilsSwipe": {
        ...
        logger.debug(f"最佳结果: {best_res}")
        logger.debug(f"页数: {page}")

        if not best_res:
            logger.debug("未找到合适结果")
            self.page = page
            return False
            ...
            for char_id_dict in char_name_dict.values():
                ...
                for res_key, res_data in char_id_dict.items():
                    if res_key == best_res:
                        ...
                        logger.debug(f"需要滑动次数: {swipe_time}")
                        raw_box = res_data.get("box")
                        found = True
                        break

        if not raw_box:
            logger.debug("未找到最佳结果对应的点击框")
            return False
